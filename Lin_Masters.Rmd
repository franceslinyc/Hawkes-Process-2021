...
For example, if we were to model the number of bus arrivals at a bus stop, the number of car accidents at a site or the requests for documents on a web server, we can consider modelling using Poisson processes.
...
In the cases of counting process and Poisson process, we assume that the arrivial rate is constant. If we have reasons to believe that the arrivial rate is not constant, we should consider modelling using nonhomogeneous Poisson processes.
...
In the example, we use the thinning slgorithm to simulate nonhomogeneous Poisson process with...  since it is the popular choice for both temporal and  spatio-temporal cases. 
