...
Let us look at a realization of a HPP in time, which we show in Figure 2. First, we note that cumulative number of points is growing at a constant linear rate. In addition, we can see that the histogram of rate appears roughly uniform. In other words, the rates are roughly constant at $\lambda = 10$. Algorithm for simulating this HPP can be found in the Appendix section. 
