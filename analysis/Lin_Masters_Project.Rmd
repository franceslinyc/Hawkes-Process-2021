...
In other words, 1. An event has to occur for it to be "counted". 2. The increments are independent, which means that the occurance of an event does not affect the probability of the occurance of another event. 3. $\lambda$ is the rate (events over time) at which points occur and is constant. 4. No more than 1 event can occur at the same location. 
...
For an HPP, the number of events, $N(t)$, are exponentially distributed. More formally, we can say that the number of events in any interval $(t, t+h]$, $N((t, t+h])$ $\sim Pos(\lambda (t+h - t))$. That is, for all $t, t+h \geq 0$ and $n = 0, 1,...$, 
