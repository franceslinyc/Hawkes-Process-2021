...
<!-- Include some citations for the examples of spatial data? Might not be 100% needed though ... but something to consider -->
...
Spatial data can be broadly categoried into three types: geostatistical (point process) data, areal data, and point pattern data. 
<!-- Maybe cite Baddeley's text here? -->
We focus on the third category in which point pattern data are realizations of spatial point processes, and questions about point pattern data typically are: Is there clustering of events? Can we define a point process that captures the events? Examples of such models include Cox and cluster processes. On the other hand, when dealing with temporal data, (marked) point processes are sometimes used interchangeable with time series, and vice versa. One major distinction, though, is that in point processes, time intervals are treated as continuous, whereas in time series, they are treated as discrete. Examples of such models include (temporal) Poisson and Hawkes processes. 

Hawkes processes are also known as self-exciting point process. More specifically, the original Hawkes processes (e.g. the ETAS model)
<!-- Here, we need to be careful because Ogata, who creates the ETAS model, essentially "riffs" on the original Hawkes process and so we might not want to label an ETAS model a "Hawkes process" per se. The lingo here is confusing, we'll talk more about this in the coming months/years. -->
are temporal, whereas the more recently developed self-exciting point processes have been extended from temporal Hawkes processes to account for both the spatial and temporal aspects of the data. The defining characteristic of Hawkes processes is that it self-excites. In other words, the occurrence of an event increases the occurrence of future events. 
<!-- To the sentence above, I think we should clarify that we see an increase in the occurrence of future events "nearby in space and time" but also that the events eventually "die out". Meaning, we don't expect events to self-excite in perpetuity. -->
In addition, more recent events exert more influence on the intensity, 
<!-- We've not yet defined what we mean by "intensity" here, so consider adding something like, "(i.e. the rate at which events occur given the history of such events)." -->
as compared to older events. For example, in seismology, an event can be an earthquake occurrence that causes aftershocks. In criminology, an event can be a gang rivalry that triggers retaliations following the gang crime. 
<!-- And in both cases, the initial event can continue to spawn "offspring" events while the "offspring" events can spawn offspring of their own. -->

In addition to modeling earthquake epicenters in seismology and crime patterns in criminology, Hawkes processes have also been used in modeling events such as forest wildfires, insurance claims, financial transcations, social network events, neuron activities, and disease spread or transmission. 
<!-- Here, we can definitely add some citations for these applications. In fact, it might be worthwhile for you to read many of these papers over the summer to start getting a sense of what sort of self-exciting point process models are in existence already. -->
Thus, it can be found in a wide variety of fields such as emergency and disaster management, insurance, finance, social network, neuroscience, and epidemiology. More recent work have been focused on modeling...... and leveraging the advancement of computing to process and model big data. 
<!-- Maybe we can say that recent work has extended the use of self-exciting point processes to novel applications, such as mass shootings (You can cite Peter's paper), but that there's still much work to be done including: Computational advances to ease the burden of using such models with bigger data sets, residual and model diagnostics, as well as methods to make the models more flexible. And for these ideas, we can cite the Souchard paper that Alex Reinhart told us about, the COVID 19 paper by George Mohler, and the article about gang violence by Junhyung Park. Let me know if you need help tracking these articles down to cite them. -->
...
<!-- Really like this paragraph. It's a nice setup for the rest of the report! -->
...
Let us begin with the counting process. A counting process counts the occurrences (or numbers) of events over time, space, space-time or, in the most general sense, any metric space in which events occur and can be counted. 
<!-- We can cite Daley and VereJones' text, "Introduction to point processes" here. We can also cite any other text where you found this very nice definition of counting processes! -->

A temporal example would be the numbers of customers arriving at a supermarket up to some time $t$. 
<!-- Be a bit more explicit with this example. What I mean is, "If we were to denote the time of arrival for customers at a super market, we would have a set of points in time in which we could count those customers over some interval of time." -->
A spatial example would be trees in different regions of a forest. 
<!-- Here too, if we consider the location of trees to occur at a point in space, then in some bounded region of the space, we could count the number of trees. I have a feeling this might spur an interest in you about measure theory and probability =) There's a ton of cool topics like, "What's a probability space? What do we mean by a "measure"? etc. All of which combines together for a really nice use of advanced mathematics! -->
A space-time example would be the numbers of earthquakes occurring in some places and during some time intervals. 
<!-- "An earthquake's epicetner would be the point in space but we also capture _when_ the point appears in time." -->
...
<!-- Any ideas on how to summarize this last point about transition probabilities in "non-stats speak"? -->
...
<!-- Since we write "space-time", we could probably write $s \time t$, but this is a small point and not really worth changing anything in your writing.  -->
but we restrict ourselves to the time domain, again. Definitions of stochastic process and point process are given as follows 
...
<!-- I really like how you're defining everything formally. We should cite where these definitions come from. So for example, we could say, "We follow with __(who ever we're citing)__ to define stochasitic, point, and counting processes. -->

<!-- \newpage -->
<!-- I'm going to comment out the newpage thing here. Not sure if you were doing it for formatting purposes or something but we can deal with formatting later. -->

Let us look at a more explicit example, which we show in Figure 1. Suppose that $N(t)$ counts the numbers of event up to some time $t$ and events occur at times $t = 0.1, 1, 1.5, 3, 5, etc$, then $N(4) = 4$ since 4 events occur in the time interval $(0, 4]$. 
<!-- Give one more example here. For instance, N(2) = 3 since events occuring at 0.1, 1, 1.5 all occur in the time interval (0, 2]. Two examples are often useful to help people see some of the finer details -->
...
The homogeneous Poisson process (HPP) is one of the simplest yet most-widely used point processes. 
<!-- ^^ This sentence is a claim, so we need to either cite someone who describes the process as such or prove this is the case ourselves (You can probably find a statement like this in the Baddeley text). -->
HPPs can be used to model the number of events such as bus arrivals at a bus stop, car accidents at a site, or the document requests on a web server over time. As we alluded to previously with counting processes, HPPs can also be considered over a space which is often taken to be a two-dimensional plane or a three-dimensional volume. 
...
<!-- \newpage -->
<!-- Here again, I'm going to comment out the newpage so that everything "flows" within the document. If we end up needing to move figures around, we can do that at the end (I can help with this as well). -->
