...
Let us turn to HPP and NPP in space first. HPP in space is also called complete spatial randomness (CSR). **CITATION** For HPP in space, the number of events $N(u) \sim Pos(\lambda |u|)$. That is, for $u \in S \subseteq R^2$, 

$$
P(N(u) = n) = \frac{(\lambda |u|)^n e^{-\lambda |u|}} {n!}
$$

where $|u|$ denotes the area of $U$. The figure on the left of Figure 3 is a realization of HPP (homogeneous Poisson process) with constant $rate = 100$. We can see that HPP points appear uniformly distributed in $U$. 

For NPP in space, the number of events $N(u) \sim Pos(\int_{u}^{} \lambda (v) dv)$. The figure in the middle 1 is a realization of NPP (nonhomogeneous Poisson process) with $intensity \ function = 400xy$. NPP points are not uniformly distributed; they are distributed according to the intensity function of the process. In this example, the points appear to concentrate at the upper-right corner. 
...
As a preview, the figure in the middle 2 and on the right is a realization of Cox process with $intensity \ function = exp(1, 1/100)$ and Matern cluster process with $kappa = 20, r = 0.05, mu = 5$, respectively. Both Cox process and Matern cluster process points appear clustered, but the way the points cluster differ. Points in Cox process cluster accordingly to some specified distribution, whereas points in cluster process cluster in some defined area. We elaborate more and discuss in details Cox and cluster process in the next section. 
