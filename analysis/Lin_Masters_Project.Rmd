...
HPP in space is also called complete spatial randomness (CSR). **CITATION** As we show in the left figure of Figure 3, HPP points are uniformly distributed in $U$. For HPP in space, the number of events $N(u) \sim Pos(\lambda |u|)$. That is, for $u \in S \subseteq R^2$, 
...
As we show in the middle 1 figure of Figure 3, NPP points are not uniformly distributed. They are distributed according to the intensity function of the process. For NPP in space, the number of events $N(u) \sim Pos(\int_{u}^{} \lambda (v) dv)$. 
