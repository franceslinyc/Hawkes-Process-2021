...
The final class of point process models we present are the Hawkes processes; they are also commonly known as self-exciting point processes **CITATION**. Like Cox and cluster process, the Hawkes process model allows for dependence between events; however, their dependence differs. In Hawkes processes, the occurrence rate of the events depends not only on time $t$ but also past events $\mathcal{H}_{t}^{N}$ up to some time $t$. This is the distinguishing feature of the Hawkes process as neither Cox nor cluster processes depends on the past history of events. It can be noted, however, that we can think of the Hawkes process as a model that incorporates the cluster process but through a conditional intensity function (Reinhart, 2018).

As previously mentioned, examples of applications for Hawkes processes include locations of earthquake epicenters, locations of crimes, and locations of patients with a communicable disease **CITATIONS**. The major sharing feature in each of these examples is that an occurrence of an event leads to an increase in the occurrence of subsequent events (i.e. self-exciting). In seismology, for example, a main quake is often followed by aftershocks. In gang crimes, one act of violence often features an act of retaliatory violence. In communicable diseases, one patient may infect others.

Since the intensity, or the expected rate in which points occur over some time and space, of the process is now as a function of past history, we refer to the expected rate as a conditional intensity function. Hawkes processes self-excites because of this conditional intensity function since conditioning on the past set of events, the expected rates at which points occur are expected to be higher when points have occurred in nearby time and space and then they gradually decline as the events get further away in time and space.

Another characteristic is that more recent events exert more influence on the intensity, as compared to older events. This characteristic is captured by the triggering part of the conditional intensity function. For example, an earthquake happens today may lead us believe that there is a higher probability of another earthquake that will occur tomorrow. However, an earthquake happened more than a year ago does not make us believe that there is a higher probability of of another earthquake that will occur tomorrow.

A Hawkes process can be uniquely identified through its conditional intensity function **CITATION: Daley and VereJones**, the formal definition of which we give below 
...
The triggering function is what makes the Hawkes process self-exciting as well as a cluster process as it allows for additional points to occur in nearby time or space before ultimately decaying back to the background rate. Typical choices of $\phi(\cdot)$ include, for example, exponentially decaying function and power-law kernel (Rizoiu, 2017), and they take the forms of
$$
