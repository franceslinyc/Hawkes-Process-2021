...
For an HPP, the number of events in any time interval $N(t)$ are Poisson distributed. More formally, we can say that the number of events in any time interval $(t, t+h]$, $N((t, t+h])$ $\sim Pos(\lambda t+h - t)$. That is, for all $t, h \geq 0$ and $n = 0, 1,...$, 
...
Before we delve further into various point processes, let us first look at a motivating example, which we show in Figure 4. We demonstrate the processes in space so that visualizations are easier to look at and comprehend. 
