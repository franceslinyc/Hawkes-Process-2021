...
HPPs are independent, stationary, and homogeneous. The distribution of the numbers of events, $N(t)$, depends only on the length of the interval.
...
1. The cumulative number of points is growing at a constant linear rate,

2. The histogram of rate appears roughly uniform, 
...
NPPs are independent but not stationary nor homogeneous. The distribution of $N(t)$ can change when shifted in $t$.

For HPP, $N(t)$ still follow a Poisson distribution but with an intensity function $\lambda(t)$ such that the rate/intensity now varies with a function of time.
...
Relaxing the independence assumption make the models even more flexible models 
...
Cox process (or doubly stochastic Poisson processes) can be viewed as a Poisson process with a random intensity function $\Lambda$. 
...
In these examples, there appears to be some observed spatial covariate (e.g. light, humidity, soil quality) or unobserved external factor that makes it more likely to observe the plants or trees preferentially in some areas.

## Cox Processes

For a Cox process, 

- We now treat this spatially varying intensity function, $\Lambda(u)$, as *random* since it relies on an underlying set of random variables, called a random field (Baddeley et al., 2015).
...
Within the general class of Cox processes, we will show a mixed Poisson process, the simplest example of all, as a demonstration. 


## Cluster Processes 

There is a significant overlap between Cox processes and cluster processes. However, while Cox processes can sometimes appear to exhibit clustering of points, cluster processes tend to make the clustering of points more explicit.

Example that can be modeled as *cluster processes* includes seedlings and saplings of California redwood (Baddeley et al., 2015).
...
For a cluster process, 

1. We first have some set of unobserved 'parent' points $\mathbf{Y}$ generated by the process. 

2. Next, each 'parent' point $y_i \in$ $\mathbf{Y}$ gives rise to a random number of 'offspring' points $z_{ij}$. 

3. These 'offspring' points $Z_{ij}$ then form a cluster process $\mathbf{X}$ but only the 'offspring' points are observed.

Within the general class of cluster processes, we will show a Matern cluster process as a demonstration.
...
Hawkes processes also allow for dependence between events. The distinguishing feature of the Hawkes processes is that they depends on past events, which is also what makes Hawkes processes 'self-excite'. 
...
Characteristics for Hawkes processes are such that

- When points have occurred in nearby time and/or space, the intensity $\lambda(t | \mathcal{H}_{t}^{})$ at which points occur are expected to be higher and then they gradually decline as the events get further away.

- More recent events exert more influence on the intensity but the intensity will fade out or decay until the next event.
...
*Here is a realization of a Hawkes process with the exponentially decaying triggering function ($\mu = 0.5, \alpha = 0.5, \beta = 0.7$). Each time the event arrives, the intensity increases and then it declines back to the background rate. The intensity for this realization gradually increases over time too.*

## Spatio-temporal SEPP (Self-Exciting Point Process)

Spatio-temporal SEPP are an extension of temporal Hawkes processes. The conditional intensity function for SEPP take the form of
$$
...
1. The triggering function is often defined to be separable in space and time.

2. The background rate here can be taken to be a function of space, which is often a NPP or constant.

3. Events that are closer to some given spatial and temporal location are more heavily influenced by the points that occur nearby in *both* space and time.
